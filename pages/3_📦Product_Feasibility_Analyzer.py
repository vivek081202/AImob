...
APP_NAME = 'Product Review & Feasibility Analysis'
...
MUST_INCLUDES = ['love', 'like', 'hate', 'cheap','good','poor','worst']
...
def main():
    st.set_page_config(
    layout="wide",
    page_title="Feasibility Analysis | AImob",
    page_icon="Images/AI.png"
    )
    st.image("Images/product.png",width = 125)
    st.title(APP_NAME)
        ...
        if len(df) == 0:
            ...
            if len(df) == 0:
                ...
                if should_display_reviews:
                    ...
                    if len(happy_df) > len(unhappy_df):
                        st.write("\n")
                        st.success(f"Product with ID {selected_product_asin} is feasible & good to buy")
                    else:
                        st.write("\n")
                        st.warning("You should try another product.")
